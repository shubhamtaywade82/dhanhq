...
module Dhanhq
  module Api
    class Base
      def initialize(connection: Dhanhq::Support::FaradayConnection.build)
      ...
      def request(method, path, params = {})
        ...
        raise Dhanhq::Errors::ClientError, "Connection error: #{e.message}"
      end
      ...
      def parse_response(response)
        ...
        when 200...300
          body
        when 400...499
          raise Dhanhq::Errors::ClientError.new(body["message"] || "Client error", response.status)
        when 500...599
          raise Dhanhq::Errors::ServerError.new("Server error: #{body["message"] || "Unknown server error"}",
                                                response.status)
        else
